...
public class Mouse {
	...
	public Mouse(final APIManager manager) {
	...
	public void move(final Point pt) {
		move(pt.x, pt.y);
	}

	public void move(final int x, final int y) {
		final Point origin = new Point(getX(), getY());
		final Point[] points = generatePath(origin, new Point(x, y));
		for (final Point point : points) {
			mouse.mouseMoved(new MouseEvent(mouse, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), 0, point.x, point.y, 0, false, 0));
		}
	}
	...
	private static Point[] generateSpline(final Point[] controls) {
		...
		final Vector<Point> spline = new Vector<Point>();
