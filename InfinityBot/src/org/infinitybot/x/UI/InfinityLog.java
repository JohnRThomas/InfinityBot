...
public class InfinityLog extends Container{
	...
	public void append(String str, Color frgrd){
	...
	public void append(final String str, final Color frgrd, final Color bkgrd){
		java.awt.EventQueue.invokeLater (new Runnable() {
			public void run() {
				...
				if(frgrd != null)line.setForeground(frgrd);
				if(bkgrd != null)line.setBackground(bkgrd);
				line.setOpaque(true);
				line.setAlignmentY(TOP_ALIGNMENT);
				line.setMaximumSize(new Dimension(800, 16));
				if(holder.getComponentCount()>=40)holder.remove(0);
				holder.add(line);
